...
uint8_t CAN_Init(CAN_TypeDef* CANx, CAN_InitTypeDef* CAN_InitStruct)
{
  ...
  else 
  {
    ...
    {
      CANx->MCR |= CAN_MCR_TTCM;  //|=  for example a|=b, which is like: a=a|b, or
    }
    else
    {
      CANx->MCR &= ~(uint32_t)CAN_MCR_TTCM; //&= for example a&=b is like this: hao a=a&b, and 
    }
...
uint8_t CAN_Transmit(CAN_TypeDef* CANx, CanTxMsg* TxMessage)
{
  ...
  if (transmit_mailbox != CAN_NO_MB)
  {
    ...
    CANx->sTxMailBox[transmit_mailbox].TIR |= TMIDxR_TXRQ;
		...
		TxMessage->TimeStampTx = (uint32_t)0x0000FFFF &  (CANx->sTxMailBox[transmit_mailbox].TDTR >> 16);  // reference in stm32_reference: CAN_RDTxR: P677
...
void CAN_Receive(CAN_TypeDef* CANx, uint8_t FIFONumber, CanRxMsg* RxMessage)
{
  ...
  RxMessage->FMI = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDTR >> 8);
	...
	/* Get the timestamp wang adds*/
	RxMessage->TimeStampRx = (uint16_t)0x0000FFFF & (CANx->sFIFOMailBox[FIFONumber].RDTR >> 16); // reference in stm32_reference: CAN_RDTxR: P680
